...
def test_create_agent(client):
    ...
    with patch.object(
        client.fastagi, "create_agent", return_value={"id": 1}
...
def test_update_agent(client):
    ...
    with patch.object(
        client.fastagi, "update_agent", return_value={"id": 1}
...
def test_pause_agent(client):
    with patch.object(
        client.fastagi, "pause_agent", return_value={"status": "paused"}
    ) as mock_pause_agent:
...
def test_resume_agent(client):
    with patch.object(
        client.fastagi, "resume_agent", return_value={"status": "resumed"}
...
def test_create_agent_run(client):
    ...
    with patch.object(
        client.fastagi, "create_agent_run", return_value={"id": 1}
...
def test_get_agent_run_status(client):
    ...
    with patch.object(
        client.fastagi, "get_agent_run_status", return_value={"status": "running"}
...
def test_get_agent_run_resources(client):
    with patch.object(
        client.fastagi, "get_agent_run_resources", return_value={"resources": []}
